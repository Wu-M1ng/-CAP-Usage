...
- 默认使用内存聚合和 SQLite 事件库；`storage_enabled: true` 时通过 `storage_path` 持久化，重启或更新插件后恢复统计并支持后端分页查询。
- 运行时元数据：页面可见当前保留策略、存储明细数、淘汰数、最近导入结果。
- 健康检查端点 `/health` 可查看插件运行状态、顶层 `alerts` 告警、SQLite 持久化状态、看板查询/缓存指标、条件请求命中率和事件导出压力指标。
