...
test('dashboard follows runtime language changes', async () => {
  ...
  assert.strictEqual(document.getElementById('trendMetric').value, 'tokens');
});
...
test('dashboard client API selection filters linked panels and sort buttons restore all data', async () => {
  ...
  await context.selectClientApiCard(selector, [{ selector, name: 's******' }]);
  ...
  assert.match(document.getElementById('clientApiFilterStatus').innerHTML, /当前筛选：s\*\*\*\*\*\*/);
