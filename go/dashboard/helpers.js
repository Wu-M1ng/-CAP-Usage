...
function maskClientAPIKey(value) { const label = String(value || '').trim(); return label ? Array.from(label)[0] + '******' : '' }
function clientApiLabel(detail) { return maskClientAPIKey(detail && detail.api_key) || t('unknown_api') }
...
function healthCellStyle(i, count, total, rate) { const columns = 96; const index = Math.max(0, Math.min(Number(i) || 0, Math.max(Number(count) - 1, 0))); const col = index % columns + 1; const row = Math.floor(index / columns) + 1; return 'grid-column:' + col + ';grid-row:' + row + ';' + (total ? 'background:' + healthColor(rate) : '') }
