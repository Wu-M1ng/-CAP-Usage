...
### 存储可靠性与请求链路优化
- 增加 SQLite 后台单写入器、批量提交、WAL 读写分离和操作超时，减少统计持久化对 API 请求完成路径的影响。
- 增加有界的 `.spool.jsonl` 写入兜底，SQLite 暂时不可写时可在重启后自动重放并去重。
- 聚合快照改为按事件数量或时间间隔保存，并使用事件 ID 水位恢复，避免每次请求生成完整快照。
- 限制 fallback 调度、流式历史 chunk、导出和前端缓存的内存占用，降低长期运行时的内存增长风险。
- 完善范围聚合、定向清理和 native/fallback 去重回归测试。

## v2.5.9 - 2026-08-04

### 请求链路与资源占用优化
- 将 SQLite 持久化、响应体解析和流式历史扫描放入有序后台队列，降低统计逻辑对 API 请求完成路径的影响。
- 限制后台任务和导出处理的内存峰值，支持二进制分块 gzip 导出，并修复异步写入后的 SQLite 聚合水位恢复。
- 修复 UTF-8 小分块、导出任务快照时间、前端缓存字节数和事件列表请求竞态问题。
- 新增对应的 Go 与 Node 回归测试，覆盖异步写入、导出和前端请求状态保护。
...
### Dashboard 全局精致化与交互升级
- **平滑样条趋势图**：使用 Hermite 单调平滑样条曲线与 35% 柔和透明渐变填充替代原本的硬折线图，新增鼠标悬浮交叉指引线（Crosshair Line）与数据节点发光。
- **毛玻璃浮窗与微型徽章**：升级 Dashboard 图表 Tooltip 为 Glassmorphism 毛玻璃风格，支持【峰值 Peak】与【均值 Avg】微型指标徽章展示。
- **动态趋势标签与百分比比重**：概览卡片新增动态环比/同比趋势徽章（如 `+14.2% ↑`），用量分布表格添加平滑进度条（Progress Pill Bar），甜甜圈图支持悬浮动态联动。
- **健康监测日历对齐**：服务健康监测热力网格左侧新增 5 天日期行对齐标签（Date Row Labels），精准标识每天的时间切片。
- **极致发光视觉与细节**：概览卡片加入弥散发光背景，平滑曲线支持 SVG 霓虹阴影滤镜，切片悬浮拥有外发光聚焦效果。

## v2.5.6 - 2026-08-03

### Dashboard 可视化与交互
- 在服务健康监测上方新增 Token 使用趋势、上游接口使用分布、端点分布和客户端 API key 使用分布图。
- Token 使用趋势支持按时间范围查看、切换可见指标和悬停查看明细；分布图提供悬停提示，便于查看具体数量和占比。
- 费用统一显示为单一指标，移除不支持的“实际消费/标准消费”切换；使用分布统一按上游接口统计。

### 统计识别与时间显示
- 规范化 `/v1/responses` 等请求端点的识别，避免兼容接口被归类为未知端点。
- 修复仅配置一个客户端 API key 时出现“未知 API key”统计的问题，并统一客户端 API key 的脱敏和聚合身份。
- 统一资源接口时间字段的时区表达，确保 Dashboard 趋势、事件和摘要使用一致的本地时间。

### SQLite 存储修复
- 修复 SQLite 事件缓存、二级索引、导入恢复和快照聚合在不同数据路径下失效的问题。
- 修复缓存成本、范围查询、裁剪明细后的聚合数据以及重复记录去重逻辑，确保重启后统计数据保持一致。

## v2.5.5 - 2026-08-01

### SQLite 存储与后端分页
- 请求事件明细和上游接口详情改为后端分页，按页查询数据，避免一次性加载大量记录。
- 使用 SQLite 替代 JSONL 事件存储，保留内存聚合数据的轻量运行时结构，降低明细数据带来的内存占用。
- 增加 SQLite 启动恢复、范围摘要、客户端 API 筛选和明细数量统计，重启后可继续展示已持久化数据。
- 移除 JSONL 存储相关配置字段；`format=jsonl` 仅作为事件导出格式保留。
- 修复数据库切换、空库复制和持久化聚合恢复逻辑。
