...
### Dashboard 健康监测与用量趋势
- 服务健康监测窗口由最近 7 天调整为最近 5 天，后端固定返回 480 个 15 分钟槽位；新版前端读取旧版 672 槽位响应时保留最新 480 个槽位
- 健康网格改为 5 行 × 96 列的响应式布局，单元格和间距随可用宽度自动调整，并保留窄屏横向滚动兜底
- 用量趋势默认选择 Token 指标；原有时间范围、小时/每日聚合规则和已保存的范围偏好保持不变

### API Key 展示与导出脱敏
- 客户端 API key 的页面显示统一为首字符加六个星号，例如 `t******`，不再显示尾部字符或追加 hash 前缀
- `/v0/management` 与 `/v0/resource` 下的摘要、事件、上游接口详情、完整用量数据及同步/后台 JSON、JSONL 导出统一应用同一脱敏规则
- 内部分组继续保留 `api_key_hash` 和不可逆 selector，selector 不再携带可逆编码的脱敏文本，避免不同真实 key 因显示文本相同而混合；当前脱敏格式导入时可保留合法的 56 位 hash，旧格式继续按兼容规则处理
